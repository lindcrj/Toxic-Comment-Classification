"""
DeBERTa model training script - Uses adversarial training and class weights
...
def train_epoch(model, dataloader, optimizer, scheduler, device, class_weight=1.0, 
    ...
    for step, batch in enumerate(progress_bar):
        ...
        # Get embedding layer - specific to DeBERTa
...
def evaluate(model, dataloader, device, class_weight=1.0):
    ...
    with torch.no_grad():
        for batch in eval_progress:
            ...
            # Update progress bar with processed samples
            eval_progress.set_postfix({"processed": len(all_preds)})
...
def read_csv_from_path(path):
    """Read CSV file from local path or S3 with progress indication"""
...
def main():
    parser = argparse.ArgumentParser(description='Train DeBERTa for toxicity classification')
    ...
    parser.add_argument('--use-adversarial', action='store_true', help='Use adversarial training')
    ...
    parser.add_argument('--enable-spot-monitoring', action='store_true', help='Enable spot instance monitoring')
    parser.add_argument('--use-spot', action='store_true', help='Running on a spot instance')
    ...
    epoch_bar = tqdm(range(args.epochs), desc="Training progress", position=0)
    
    for epoch in epoch_bar:
        # Update progress bar description
        epoch_bar.set_description(f"Epoch {epoch+1}/{args.epochs}")
        
        # Train with progress bar inside train_epoch function
        ...
        # Update epoch progress bar with loss
        epoch_bar.set_postfix({"train_loss": f"{train_loss:.4f}"})
        
        # Validate with progress bar inside evaluate function
    ...
    # Test with progress bar inside evaluate function
    ...
    results_path = f"{args.model_dir}/deberta_results.json"
